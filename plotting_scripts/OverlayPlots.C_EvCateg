...
for(int i_data=0; i_data<3; i_data++)
  {
    ...
    sprintf(full_path1,"%s/overlay_resolution_%s.png",path2,data[i_data]);
    sprintf(full_path2,"%s/overlay_response_%s.png",path2,data[i_data]);
    ...
    h_resolution->SetMarkerColorAlpha(kPink+2, 0.95);
...
gPad->Modified();
 ...
 legend->Draw("sames");
  ...
  gPad->Modified();
  gPad->Update();
  h_response1->GetHistogram()->GetYaxis()->SetRangeUser(0.7,1.3);
