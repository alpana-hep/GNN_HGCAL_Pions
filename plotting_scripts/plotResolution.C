...
char* full_path = new char[2000];
  ...
  sprintf(path2,"Results/v1",filename.c_str());
  ...
  int rebin=2;
  TFile* file = new TFile("PionResolution_5M_lr1e04_fixwt_scalTB.root","RECREATE");
 ...
 int Elist[8]={20,50,80,100,120,200,250,300};
  for (int i_data=0; i_data<3;i_data++)
      ...
      for(int i_en=0;i_en<8;i_en++)
	{
	  ...
	  sprintf(hist_name,"%s_Predi_TrueEn_%d",data[i_data],Elist[i_en]);//,Elist[i_en]+4);
