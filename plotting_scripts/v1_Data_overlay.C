...
const char *eve_cat[2] = { "SSinEE", "MipsInEE"};
 ...
 //      sprintf(full_path2,"%s/%s_overlay_response_v1.png",path2);
  ...
  //       {
 sprintf(full_path1,"%s/overlay_resolution_updated.png",path2);//,eve_cat[i]);
 sprintf(full_path2,"%s/overlay_response_updated.png",path2);//,eve_cat[i]);
TMultiGraph* mg = new TMultiGraph();
  TMultiGraph* mg1 = new TMultiGraph();
auto legend = new TLegend(0.55,0.6,0.9,0.9);
  legend->SetHeader("","C");
  gStyle->SetLegendTextSize(0.04);
  auto legend1 = new TLegend(0.55,0.1,0.9,0.35);
  legend1->SetHeader("","C");
...
TGraphErrors* h_resolution_shubham = (TGraphErrors*)inputfile1->Get(hist_name9);
    ...
    h_response_shubham->SetMarkerColorAlpha(kBlue+2, 0.95);
    h_response_shubham->SetMarkerSize(4);
    h_response_shubham->SetMarkerStyle(32);
    h_response_shubham->SetLineColor(kBlue+2);
    h_response_shubham->SetLineStyle(2);
    h_response_shubham->SetLineWidth(2);

    mg->Add(h_resolution_shubham);
    mg1->Add(h_response_shubham);
    legend->AddEntry(h_resolution_shubham,"DN-21-012","p");
    legend1->AddEntry(h_resolution_shubham,"DN-21-012","p");
