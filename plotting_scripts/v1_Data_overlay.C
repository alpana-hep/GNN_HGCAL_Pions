...
const char *eve_cat[2] = { "SSinEE", "MipsInEE"};
 ...
 //      sprintf(full_path2,"%s/%s_overlay_response_v1.png",path2);
  ...
  //       {
 sprintf(full_path1,"%s/overlay_resolution_v2_afterfit.png",path2);//,eve_cat[i]);
 sprintf(full_path2,"%s/overlay_response_v2_afterfit.png",path2);//,eve_cat[i]);
TMultiGraph* mg = new TMultiGraph();
  TMultiGraph* mg1 = new TMultiGraph();
...
for (int i_data=0; i_data<5;i_data++)
    {
      ...
      sprintf(full_path3,"%s/overlay_resolution_%s_afterfit.png",path2,data[i_data]);
      //      sprintf(full_path4,"%s/overlay_response_%s_afterfit.png",path2,data[i_data]);
      ...
      TF1* fit_func = new TF1("reso","sqrt([0]*[0]+[1]*[1]/x+[2]*[2]/(x*x))",20,300);
      fit_func->SetParameters(0.06,1.20,0.);
      
      fit_func->SetLineColor(kBlack);//point_col[ii]);
      fit_func->SetLineWidth(2);
      fit_func->SetLineStyle(2);
      h_resolution_valid->Fit(fit_func,"","R",15,330);
