...
#include<iostream>
using namespace std;
void DataOverlay(string file_name, string file_name_test)//, string file_name_data)
{
 ...
 sprintf(hname1,"%s",file_name_test.c_str());
  ...
  sprintf(full_path1,"%s/overlay_resolution_v1.png",path2);
  sprintf(full_path2,"%s/overlay_response_v1.png",path2);
...
TGraphErrors* h_resolution_shubham = (TGraphErrors*)inputfile1->Get(hist_name9);
 ...
 legend1->AddEntry(h_resolution_shubham,"Previous studies","p");
 gStyle->SetLegendTextSize(0.05);
  mg1->Draw("LP");
