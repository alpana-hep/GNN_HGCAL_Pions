...
void EventCat_plotResolution(string filename)
{
...
char* full_path = new char[2000];
  ...
  sprintf(path2,"Results/Event_categ");//,filename.c_str());
  ...
  int rebin=2;
  TFile* file = new TFile("PionResolution_flaten_nosemi_22epoch_5M_lr3e03_absloss_Noscaled_EvtCat.root","RECREATE");
 ...
 for (int i=0;i<2;i++)
  ...
  for (int i_data=0; i_data<5;i_data++)
    {
      sprintf(reso_name,"Resolution_%s_%s",eve_cat[i],data[i_data]);
      sprintf(resp_name,"Response_%s_%s",eve_cat[i],data[i_data]);
      ...
      // h_resolution->GetAttLine(0)->SetLineColor(kRed);
      // h_resolution->GetAttLine(1)->SetLineColor(kBlue);
      ...
      for(int i_en=0;i_en<8;i_en++)
	{
	  ...
	  sprintf(hist_name,"%s_%s_Predi_TrueEn_%d",eve_cat[i],data[i_data],Elist[i_en]);//,Elist[i_en]+4);
	  cout<<hist_name<<endl;
	  ...
	  resp->SetLineWidth(2);
	  sprintf(full_path,"%s/%s_%s_PredEn_%03d.png",path2,data[i_data],eve_cat[i],Elist[i_en]);
