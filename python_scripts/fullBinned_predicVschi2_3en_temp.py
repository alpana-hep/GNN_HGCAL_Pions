...
valid_idx_file="/home/rusack/shared/pickles/HGCAL_TestBeam/pkl_files/pkl_Sim_50100300_hadinfor/trimAhcal/all_valididx.pickle"
train_idx_file="/home/rusack/shared/pickles/HGCAL_TestBeam/pkl_files/pkl_Sim_50100300_hadinfor/trimAhcal/all_trainidx.pickle"
...
fout= ROOT.TFile("hist_updatedAbscateg_trimAhcal_86epochs_3en_lr1e04_leakVsabs_.root", 'RECREATE')
fout.cd()
h2d_trueEnvstrueEn =[]
h2d_chi2vs_model =[]
h2d_chi2vs_true = []
...
for i_hist in range(len(Energy)):
    ...
    xhigh = Energy[i_hist]
    h2d_gnnVspi0.append(ROOT.TH2F("h2d_gnnVspi0_%s"%name1,"",500,0,xhigh,500,0,Energy[i_hist]))
    h2d_chi2vspi0.append(ROOT.TH2F("h2d_chi2vspi0_%s"%name1,"",500,0,xhigh,500,0,Energy[i_hist]))
    h2d_Absvspi0.append(ROOT.TH2F("h2d_Absvspi0_%s"%name1,"",500,0,Energy[i_hist],500,0,Energy[i_hist]))
    ...
    h1d_totalleak.append(ROOT.TH1F("h1d_totalleak_%s"%name1,"",500,0,Energy[i_hist]))
    ...
    for i_b in range(len(baseline)):
        ...
        temp_gnn.append(ROOT.TH2F("h2d_Gnnvs_%s"%name,"",500,0,xhigh,200,0,Energy[i_hist]))
        temp_chi2.append(ROOT.TH2F("h2d_Chi2vs_%s"%name,"",500,0,xhigh,200,0,Energy[i_hist]))
        temp_pi0.append(ROOT.TH2F("h2d_leadpi0Vs_%s"%name,"",500,0,Energy[i_hist],500,0,Energy[i_hist]))
    ...
    for i_b in range(len(baseline1)):
        ...
        temp_gnn.append(ROOT.TH2F("h2d_Gnnvs_%s"%name,"",500,0,xhigh,500,0,Energy[i_hist]))
        temp_chi2.append(ROOT.TH2F("h2d_Chi2vs_%s"%name,"",500,0,xhigh,500,0,Energy[i_hist]))
        temp_pi0.append(ROOT.TH2F("h2d_leadpi0Vs_%s"%name,"",500,0,Energy[i_hist],500,0,Energy[i_hist]))
    ...
    for i_b in range(len(baseline2)):
        ...
        temp.append(ROOT.TH2F("h2d_%s"%name,"",500,0,xhigh,500,0,Energy[i_hist]))
...
for i_en in range(len(Energy)):
    ...
    for i in range(len(trueEn_pkl)):
        ...
        if(trueEn_pkl[i]>=(Energy[i_en]-2) and trueEn_pkl[i]<=(Energy[i_en]+2)):
            ...
            h2d_leakvsChi2[i_en][0].Fill(Pred_[i],energyLeakResidual_[i])
            h2d_leakvsChi2[i_en][1].Fill(Pred_[i],Total_EE[i])
            h2d_leakvsChi2[i_en][2].Fill(Pred_[i],Total_FH[i])
            h2d_leakvsChi2[i_en][3].Fill(Pred_[i],energyLeakTransverseAH_[i])
            h2d_leakvsChi2[i_en][4].Fill(Pred_[i],energyLeakLongitudinal_[i])
...
for i_hist in range(len(Energy)):
    ...
    for i_c in range(len(baseline0)):
        temp_gnn=[]
        temp_chi2=[]
        h2d_gnnVspi0_categ_temp.append(ROOT.TH2F("h2d_gnnVspi0_%s_%s"%(baseline0[i_c],name1),"",500,0,xhigh,500,0,Energy[i_hist]))
        h2d_chi2vspi0_categ_temp.append(ROOT.TH2F("h2d_chi2vspi0_%s_%s"%(baseline0[i_c],name1),"",500,0,xhigh,500,0,Energy[i_hist]))
        h2d_Absvspi0_categ_temp.append(ROOT.TH2F("h2d_Absvspi0_%s_%s"%(baseline0[i_c],name1),"",500,0,Energy[i_hist],500,0,Energy[i_hist]))
        ...
        h1d_totalleak_categ_temp.append(ROOT.TH1F("h1d_totalleak_%s_%s"%(baseline0[i_c],name1),"",500,0,Energy[i_hist]))
...
# mean_ = Mean_sigma[1]
# sigma_ = Mean_sigma[2]
energy_,Mean_,Sigma_ = np.loadtxt('./FracAbs_meanSigma_gausFit_8Enpoints.txt',usecols=(0,1, 2), unpack=True)
