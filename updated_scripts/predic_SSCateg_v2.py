...
valid_idx_file="/home/rusack/shared/pickles/HGCAL_TestBeam/pkl_files/pkl_LeakageSmaples_8beamEn/all_valididx.pickle"
train_idx_file="/home/rusack/shared/pickles/HGCAL_TestBeam/pkl_files/pkl_LeakageSmaples_8beamEn/all_trainidx.pickle"
...
tb_valid_idx_file="/home/rusack/shared/pickles/HGCAL_TestBeam/pkl_files/test_tb_fix_raw_ahcalTrim/all_valididx.pickle"
tb_train_idx_file="/home/rusack/shared/pickles/HGCAL_TestBeam/pkl_files/test_tb_fix_raw_ahcalTrim/all_trainidx.pickle"
...
print(tb_preds_ratio[tb_preds_ratio>3])
tbfolder = sys.argv[4]
tb_trueEn= "%s/beamEn.pickle"%tbfolder
...
#SSLocation_file_sim = "%s/SsLocation.pickle"
...
#fout= ROOT.TFile("hist_8beamEn_SSevtCateg_ep104_dylr_5M_trimAHcal_4Aggre.root", 'RECREATE')
...
for i in range(len(trueEn_pkl)):#valid_idx)):
    valid_trueEn=(trueEn_pkl[[i]])
    valid_predEn=(preds_trueEn[[i]])
    SS_location= SSLocation_arr_sim[[i]]
    ...
    for i_sloc in range(m):
        if(i_sloc<5):
            if(SS_location>(SS_location_arr[i_sloc]-6) and SS_location<=(SS_location_arr[i_sloc])):               
                for ibin in range(8):
                    if(valid_trueEn==Energy[ibin]-2): # and valid_trueEn<=Energy[ibin] ):
            ...
            if(SS_location>(SS_location_arr[i_sloc]-3) and SS_location<=(SS_location_arr[i_sloc])):
                for ibin in range(8):
                    if(valid_trueEn==Energy[ibin]):# and valid_trueEn<=Energy[ibin]+2 ):
                        hist_valid_predVsSS[ibin][i_sloc].Fill(valid_predEn)
